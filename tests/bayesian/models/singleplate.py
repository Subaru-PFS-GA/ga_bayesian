...
class SinglePlate(Model):
    ...
    def model(self, context):
        ...
        with context.plate('n', N):
            ...
            x_2 = context.sample('x_2', Normal(x_1, theta_2.expand_as(x_1)), observed=True)
    ...
    def step(self, context, state):
        context.step(
            ...
            proposal = MultivariateNormalProposal(
                torch.stack([ self.theta_1.value(state), self.theta_2.value(state) ], dim=-1),
        ...
        context.step(
            ...
            [ self.x_1, self.x_2 ],
            proposal = MultivariateNormalProposal(
                torch.stack([ self.x_1.value(state), self.x_2.value(state) ], dim=-1),
                torch.tensor([ 1.0 ]) * torch.eye(2)
            )
        )

...
class Mixture(Model):
    ...
    def step(self, context):
        ...
        context.step(
            'theta_1',
            [ self.theta_1 ],
            proposal = NormalProposal(self.theta_1.value(context.state), 0.5)
        )
        
        context.step(
            'theta_2',
            [ self.theta_2 ],
            proposal = NormalProposal(self.theta_2.value(context.state), 0.5)
        )
        ...
        context.step(
            'x_1',
            [ self.x_1 ],
            proposal = NormalProposal(self.x_1.value(context.state), 1.0)
        )
        
        context.step(
            'x_2',
            [ self.x_2 ],
            proposal = NormalProposal(self.x_2.value(context.state), 3.0)
        )
